...
using namespace cv;
using namespace std::
    chrono_literals; //可以使用数字加单位的表示法，如：1ms，需要
                     // C++14 标准
...
public:
  Vision(std::string name) : Node(name) {
    ...
    // 创建一个发布器
    publisher_ =
        this->create_publisher<sensor_msgs::msg::Image>("robot_image", 1);

    // 创建一个订阅器，并指定回调函数
    subscription_ = this->create_subscription<std_msgs::msg::Int32>(
        "camera_control", 1,
        std::bind(&Vision::controlcallback, this, std::placeholders::_1));
    ...
    if (is_simulation_) {
      sim_subscription_ = this->create_subscription<sensor_msgs::msg::Image>(
          "/robot_image_sim", 10,
          std::bind(&Vision::sim_image_callback, this, std::placeholders::_1));
      ...
      timer_ = this->create_wall_timer(
          ...
          std::bind(
              &Vision::publishImage,
              this)); // 10ms-100帧/s,20ms-50帧/s,实际帧率略小，因为ROS2传输较大的图片消息会产生延迟，可考虑图片压缩传输(暂未实现)
    }
  }
  ...
  bool OpenCamera() {
    ...
    try {
      ...
      if (bInit_) {
        ...
        emStatus =
            GXIsImplemented(g_hDevice, GX_DS_INT_STREAM_TRANSFER_NUMBER_URB,
    ...
    try {
      ...
      if (pFrameBuffer->nStatus != GX_FRAME_STATUS_SUCCESS) {
        ...
        if (nRet == PIXFMT_CVT_SUCCESS) {
          cv::Mat imgtmp = cv::Mat(pFrameBuffer->nHeight, pFrameBuffer->nWidth,
                                   CV_8UC3, g_pRGBImageBuf);
          //图片尺寸修改，原始图像1920x1200
          cv::resize(imgtmp, imgtmp,
                     cv::Size(imgtmp.cols * 0.5, imgtmp.rows * 0.5), 0, 0,
                     cv::INTER_LINEAR);
          //色彩空间转化
          cv::cvtColor(imgtmp, imgtmp,
                       cv::COLOR_RGB2BGR); //参数1-要处理的，参数2-处理后的
  ...
  int PixelFormatConvert(PGX_FRAME_BUFFER pFrameBuffer) {
    ...
    case GX_PIXEL_FORMAT_BAYER_BG8: {
      ...
      emDXStatus = DxRaw8toRGB24(
          (unsigned char *)pFrameBuffer->pImgBuf, g_pRGBImageBuf,
          pFrameBuffer->nWidth, pFrameBuffer->nHeight, RAW2RGB_NEIGHBOUR,
    ...
    case GX_PIXEL_FORMAT_BAYER_BG12: {
      ...
      emDXStatus = DxRaw16toRaw8((unsigned char *)pFrameBuffer->pImgBuf,
                                 g_pRaw8Image, pFrameBuffer->nWidth,
...
private:
  ...
  rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr
      sim_subscription_; // for sim image
  cv::Mat image_;
  int img_counter;
  ...
  void publishImage() {
    ...
    if (bPub_) {
      ...
      { // 采集后 resize 为960x600 并将格式转化为 BGR 保存在image_中
        ...
        // 创建一个ROS消息对象
        // 使用cv_bridge库将OpenCV图像image_转换为ROS图像消息并发布
        ...
        //测试图像发布是否正常
        // RCLCPP_INFO(this->get_logger(),
        // "第%ld次发布采集图像完成,高度=%d,宽度=%d", img_counter, image_.rows,
        // image_.cols);
      } else {
  ...
  void sim_image_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
    if (bPub_) {
      ...
      if (duration.seconds() >= 1.0) {
        double fps = img_counter / duration.seconds();
        RCLCPP_INFO(this->get_logger(),
                    "\033[32m\n 图像采集正常! 当前帧率：%.2f FPS \033[0m", fps);
        last_frame_time_ = current_time;
        img_counter = 0;
      }

      // 🔹 1. ROS Image → cv::Mat（RGB）
      ...
      // 🔹 3. 转回 ROS Image（bgr8）
      auto out_msg = cv_bridge::CvImage(msg->header, // 保留原 header
                                        "bgr8", bgr_image)
                         .toImageMsg();
